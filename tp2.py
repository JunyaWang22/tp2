...
def td(contenu, id_pos): 
    return '<td' + ' id="' + 'case' + str(id_pos) + '" onclick="clic(' + str(id_pos) + ')"' + '>' + contenu + '</td>'
...
def clic(id_pos):   
    ...
    carte = document.querySelector(case(id_pos)).innerHTML
    ...
    if id_pos in index_lime:
        document.querySelector(case(index+1)).innerHTML = '<img src="cards/' + a_change + '">'
        document.querySelector(case(id_pos)).innerHTML = '<img src="cards/absent.svg">'
        document.querySelector(case(id_pos)).removeAttribute("style")
        trouver_lime(id_pos)
...
def brassage(liste):
    ...
    while essai <= 3:
        aligne()
        for _ in range (ROW):
            cartei=brassage(column[(i+1):])
            for j in cartei:
                column[:i].append(CARTES[j])
    essai += 1
    
    msg = "Vous pouvez encore "
    html_brassage = '''<button id="brasser" onclick="brassage(column[(i+1):])">Brasser les cartes </button>'''
    msg1 = str(essai) + "fois\n"
...
racine.innerHTML = css + init() #+ brassage(column[(i+1):]) + html_redemarre
lime()
